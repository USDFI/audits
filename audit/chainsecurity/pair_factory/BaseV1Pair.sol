...
contract BaseV1Pair {
    ...
    bytes32 internal constant PERMIT_TYPEHASH =
        0x516271bbd510671c286a2d724913baaeefeed98a8db417ad43b190435e6372eb;
    ...
    modifier gasThrottle() {
        if (BaseV1Factory(factory).shouldGasThrottle() == true) {
            require(
                tx.gasprice <= BaseV1Factory(factory).maxGasPrice(),
                "gas is too high!"
            );
        }
        _;
    ...
    constructor() {
        ...
        if (_stable) {
            ...
            fee = BaseV1Factory(factory).baseStableFee();
        } else {
            name = string(
                abi.encodePacked(
                    "USDFI.com VolatileV1 AMM - ",
                    IERC20(_token0).symbol(),
                    "/",
                    IERC20(_token1).symbol()
                )
            ...
            fee = BaseV1Factory(factory).baseVariableFee();
        }
    ...
    function usdfiMaker() external view returns (address) {
        return BaseV1Factory(factory).usdfiMaker();
    }

    function protocol() external view returns (address) {
        return BaseV1Factory(factory).protocolAddresses(address(this));
    }
    ...
    function claimFees() external returns (uint256 claimed0, uint256 claimed1) {
        ...
        if (claimed0 > 0 || claimed1 > 0) {
            ...
            (claimed0, claimed1) = BaseV1Fees(fees).claimFeesFor(
                msg.sender,
                claimed0,
                claimed1
            );
    ...
    function setFee(uint256 _fee) external {
        require(
            msg.sender == BaseV1Factory(factory).feeAmountOwner() ||
                msg.sender == BaseV1Factory(factory).admin(),
