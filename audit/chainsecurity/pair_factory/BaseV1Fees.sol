...
contract BaseV1Fees {
    ...
    function claimFeesFor(
        ...
        address protocolAddress = BaseV1Factory(factory).protocolAddresses(
            pair
        );
        if (protocolAddress != address(0x0) && protocolFee > 0) {
            if (amount0 > 0)
                _safeTransfer(
                    token0,
                    protocolAddress,
                    (amount0 * protocolFee) / 1000
                );
            if (amount1 > 0)
                _safeTransfer(
                    token1,
                    protocolAddress,
                    (amount1 * protocolFee) / 1000
                );
        }

        // send X% to usdfiMaker
        address usdfiMaker = BaseV1Factory(factory).usdfiMaker();
        if (usdfiMaker != address(0x0)) {
            if (amount0 > 0)
                _safeTransfer(
                    token0,
                    usdfiMaker,
                    (amount0 * usdfiMakerFee) / 1000
                );
            if (amount1 > 0)
                _safeTransfer(
                    token1,
                    usdfiMaker,
                    (amount1 * usdfiMakerFee) / 1000
                );
        }
